...
$Expiry->add(DateInterval::createFromDateString($GLOBALS["LongTokenExpiry"]));
if ($CurrentTime < $Expiry) { $this->UpdateSessionToken();
  return $this->Token;
  } else {
  ...
  if (abs($Now->getTimestamp() - $LastTry->getTimestamp()) < $GLOBALS["SIGNIN_DELAY"]) { throw new TooManyRequestsException("Please wait before you can update sessiontoken. ". abs($Now->getTimestamp() - $LastTry->getTimestamp()));
      ...
      $Updater = $Connection->prepare(" Update `accounts` set `LastSigninAt`=:LoginDateTime,`SessionToken`=:SessionToken,`LastActivityAt`=:LastActivityAt WHERE UserID=:UserID"); $Token=bin2hex(openssl_random_pseudo_bytes(32)); $LoginDateTime=new DateTime("now", new DateTimeZone($GLOBALS["DefaultTimeZone"])); $Updater->bindValue(":LastActivityAt", $LoginDateTime->format("Y-m-d H:i:s"), PDO::PARAM_STR);
    ...
    */
    ...
    for ($TargetIndex = 0; $TargetIndex < count($Result); $TargetIndex++) { if ($Result[$TargetIndex]["Revision"]===$Revision) { $MaxIndex=$TargetIndex; $FoundFlag=true; break; } if ($Revision===null) { if ($Result[$TargetIndex]["Revision"]> $MaxRev) {
